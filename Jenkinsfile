pipeline {
    agent any
    environment {
        REPO_URL = 'git@github.com:thduc123/SEP392_CAPSTONE_PROJECT_SPRING2025_ZMSOS_BE.git'
    ...
    stages {
        ...
        stage('Determine Running Docker Image') {
            steps {
                script {
                    def runningImage = sh(script: "ssh ${env.DEPLOY_USER}@${env.DEPLOY_SERVER} 'docker ps --format \"{{.Image}}\" | grep ${env.IMAGE_NAME} || echo \"\"'", returnStdout: true).trim()
